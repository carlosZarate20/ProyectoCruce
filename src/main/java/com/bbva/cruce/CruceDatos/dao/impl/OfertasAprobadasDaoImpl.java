package com.bbva.cruce.CruceDatos.dao.impl;
...
public class OfertasAprobadasDaoImpl implements OfertasAprobadasDao{
	...
	public void cruceOfertas(int identificador) throws Exception {
		...
		try{
			...
			simpleJdbcCall.withProcedureName("spCruceOfertasAprobadas")
			  			  .declareParameters( new SqlParameter("ID_CARGA_CABECERA", Types.INTEGER))
			  			  .declareParameters( new SqlOutParameter("RESULTADO", Types.INTEGER));
			
			SqlParameterSource in = new MapSqlParameterSource()
					.addValue("ID_CARGA_CABECERA", identificador,Types.INTEGER);
			
			Map<?, ?> map = simpleJdbcCall.execute(in);
			result = (Integer) map.get("RESULTADO");
	
		}catch(Exception e){
			throw new Exception("No se puede realizar el cruce de ofertas");
		}
		
		if(result == 0) {
			System.out.println("Error en el spCruceOfertasAprobadas");
		}		
